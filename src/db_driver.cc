...
inline int countNonEmptyLines(const std::string& text) {
    int lineCount = 0;
    bool inEmptyLine = true;
    ...
    // Check for the last line if it's not empty
    if (!inEmptyLine) {
        lineCount++;
    }
...
/* Error reporting to forkserver controller */
inline void make_dir(std::string s){
...
static void __afl_map_shm(void) {
  ...
  if (__afl_map_size > MAP_SIZE) {
    if (__afl_map_size > FS_OPT_MAX_MAPSIZE) {
      ...
      fprintf(stderr,
              "Warning: AFL++ tools 12345 will need to set AFL_MAP_SIZE to %u to "
              "be able to run this instrumented program!\n",
              __afl_map_size);
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  std::string cuckoo_conn = database->cuckoo_conn();

  /* This is were the testcase data is written into */
  constexpr size_t kMaxInputSize = 0x100000;
  u8 *buf = (u8 *)malloc(kMaxInputSize);  // this is the maximum size for a test case! set it!
  s32 len;
  ...
  while ((len = __afl_next_testcase(buf, kMaxInputSize)) > 0) {
    ...
    std::string op = (char*)buf;
    op.resize(len);

    // all_query_case += 1;

    // translate op to plan (Haskell)
    // std::cout << "Cuckoo conn : " << cuckoo_conn << std::endl;
    char * sql = operator2PlannedStmt(op, cuckoo_conn);

    // failed to fill plan, get next case
    if(sql == NULL){
      std::cout << op << std::endl;
      std::cout << "\033[" << "31m" << "ERROR: " << "haskell: error!" << "\033[0m" << std::endl;
      database->clean_up_env();
      __afl_end_testcase(client::kSyntaxError);
      continue;
    }
    
    client::ExecutionStatus status = database->execute((const char *)sql, strlen(sql));
    ...
    if(status == client::kNormal){
      std::cout << "\033[" << "34m" << "Execution succeed!" << "\033[0m" << std::endl;
    }else if(status == client::kExecuteError){
      std::cout << "\033[" << "31m" << "Execution failed!" << "\033[0m" << std::endl;
    }else if(status == client::kServerCrash){
        std::cout << "\033[" << "32m" << "Server crashed!" << "\033[0m" << std::endl;
    }
